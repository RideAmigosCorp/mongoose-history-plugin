...
const CompiledSchema = new Schema({ name: String, size: String });
...
const EmbeddedSchema = new Schema({ name: String, size: String });
...
describe('Mongoose History Plugin', () => {
    ...
    test('should add the plugin to a schema', async () => {
        const TestSchema = new Schema({ name: String, size: String });
        // Track if plugin was called
        let pluginCalled = false;
        const wrappedPlugin = (schema) => {
            pluginCalled = true;
    ...
    test('should create history for sub documents', async () => {
        const parentSchema = new Schema({ tanks: [EmbeddedSchema] });
